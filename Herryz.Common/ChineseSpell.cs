...
namespace Herryz.Common
{
	public class ChineseSpell
	{
		...
		public static string GetFirst(string strText)
		{
			int length = strText.Length;
			string text = "";
			for (int i = 0; i < length; i++)
			{
				text += ChineseSpell.getSpell(strText.Substring(i, 1));
			}
			return text;
		}
			...
			for (int i = 0; i < array2.Length; i++)
			{
				if (regex.IsMatch(array2[i].ToString()))
				{
					...
					else
					{
						...
						{
							for (int j = ChineseSpell.pyValue.Length - 1; j >= 0; j--)
							{
								if (ChineseSpell.pyValue[j] <= num3)
								{
									text += ChineseSpell.pyName[j];
									break;
								}
							}
						}
